...
import (
	...
	"github.com/weijun-sh/gethscan/tokens"
	"github.com/weijun-sh/gethscan/tokens/btc"
	"github.com/weijun-sh/gethscan/tokens/btc/electrs"
	"github.com/weijun-sh/gethscan/tokens/eth"
	"github.com/weijun-sh/gethscan/tokens/tools"
...
var (
	ElctrsBlocks = 25 //elctrs API defined
)
...
func (scanner *btcSwapScanner) scanLoop(from uint64) {
	...
	for {
		...
		scanner.doScanRangeJob(from+1, latest+1)
...
func (scanner *btcSwapScanner) doScanRangeIndexJob(blockHash string, height, start, end uint64) {
	if start >= end {
		return
	}
	jobs := scanner.jobCount
...
func (scanner *btcSwapScanner) doScanIndex(blockHash string, height, job, startIndex, txCount uint64, wg *sync.WaitGroup) {
	...
	for startIndex < txCount {
		...
		for i := 0; i < scanner.rpcRetryCount; i++ {
			...
			txs, err = scanner.bridge.GetBlockTransactions(blockHash, uint32(startIndex))
			if err == nil {
				break
			}
...
func (scanner *btcSwapScanner) processTx(tx *electrs.ElectTx) {
	txid := *tx.Txid
	err := scanner.bridge.CheckSwapinTxType(tx)
	if err != nil {
		return
	}
	log.Info("post p2sh router register", "txid", txid)
	args := map[string]interface{}{
		"chainid": "1000004346947",
		"txid": txid,
	}
	var result interface{}
	for i := 0; i < scanner.rpcRetryCount; i++ {
		err = client.RPCPost(&result, scanner.swapServer, "swap.RegisterRouterSwap", args)
		if err == nil {
			log.Info("post p2sh router register successed", "txid", txid, "err", err)
			break
		}
		...
		log.Warn("post p2sh router register failed", "txid", txid, "err", err)
	}
}
